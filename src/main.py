...
def parse_args():
    ...
    # 数据参数
    parser.add_argument(
        "--data_path", type=str, default="mnist-demo.csv", help="MNIST数据集文件路径"
    )
    ...
    # 模型参数
    parser.add_argument("--model_path", type=str, default="models", help="模型保存目录")
    parser.add_argument(
        "--model_name", type=str, default="mnist_cnn.pth", help="模型保存文件名"
    )
    ...
    parser.add_argument(
        "--visualize_samples", type=int, default=10, help="可视化样本数量"
    )
...
def main():
    ...
    if args.mode == "train":
        ...
        # 可视化一些预测结果
        visualize_predictions(
            model, val_loader, num_samples=args.visualize_samples, device=device
        )
